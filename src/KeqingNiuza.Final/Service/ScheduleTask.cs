...
internal static class ScheduleTask
{
    ...
    private static string GetQueryXml()
    {
        ...
        if (launcherPath == null) launcherPath = Registry.GetValue(regepath_sea, "DisplayIcon", null) as string;
    ...
    *[System[band(Keywords, 9007199254740992) and
    ...
    public static void SendNotification()
    {
        if (File.Exists($"{UserDataPath}\\ScheduleTask.json"))
        {
            ...
            {
                if (info.TriggerType == ScheduleInfoTriggerType.None) continue;
    ...
    public static void AddRealTimeNotifacation(IEnumerable<ScheduleInfo> list)
    {
        ...
        foreach (var item in scheduledNofications) notifer.RemoveFromSchedule(item);
        foreach (var info in list)
        {
            if (info.TriggerType == ScheduleInfoTriggerType.None) continue;
            if (info.TriggerType == ScheduleInfoTriggerType.Recovery)
