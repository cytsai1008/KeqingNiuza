...
public class WishOriginalDataViewModel : INotifyPropertyChanged
{
    ...
    public void FilterWishData(bool delay = false)
    {
        ...
        switch (SelectedWishType)
        {
            ...
            case "角色活动祈愿":
                tmp = tmp.FindAll(x =>
                    x.WishType == WishType.CharacterEvent || x.WishType == WishType.CharacterEvent_2);
                break;
            case "武器活动祈愿":
        ...
        if (SelectedItemType != "-") tmp = tmp.FindAll(x => x.ItemType == SelectedItemType);
        if (SelectedItemRank != "-") tmp = tmp.FindAll(x => x.Rank == int.Parse(SelectedItemRank));
        if (SelectedWishEvent.Name != "---")
        {
            if (SelectedWishEvent.WishType == WishType.CharacterEvent)
                tmp = tmp.FindAll(x =>
                    (x.WishType == WishType.CharacterEvent || x.WishType == WishType.CharacterEvent_2) &&
                    x.Time > SelectedWishEvent.StartTime && x.Time < SelectedWishEvent.EndTime);
            else
                tmp = tmp.FindAll(x =>
                    x.WishType == SelectedWishEvent.WishType && x.Time > SelectedWishEvent.StartTime &&
                    x.Time < SelectedWishEvent.EndTime);
        }
    ...
    {
        get => _ToggleButton_Search_IsChecked;
        set
        {
