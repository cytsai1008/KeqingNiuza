...
public class MidiTrack
{
    ...
    {
        CanPlayNoteNumber = Track.Events.Where(x => x.EventType == MidiEventType.NoteOn).Count(x =>
            Const.NoteToVisualKeyDictionary.ContainsKey((x as NoteOnEvent).NoteNumber + noteLevel));
        if (CanBeChecked)
        {
            MaxNoteLevel = Track.Events.Where(x => x.EventType == MidiEventType.NoteOn)
                .Max(x => (x as NoteOnEvent).NoteNumber + noteLevel);
            MinNoteLevel = Track.Events.Where(x => x.EventType == MidiEventType.NoteOn)
                .Min(x => (x as NoteOnEvent).NoteNumber + noteLevel);
        }
    }
}