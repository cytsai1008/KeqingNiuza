...
public class MidiPlayer
{
    ...
    {
        get => _playback.GetCurrentTime<MetricTimeSpan>();
    ...
    public double Speed
    {
        ...
        set
        {
            if (_playback == null || IsPlaying) return;
            if (value > 0) _playback.Speed = value;
        }
    }
    ...
    private void NoteEventPlayed(object sender, MidiEventPlayedEventArgs e)
    {
        if (e.Event.EventType == MidiEventType.NoteOn)
        {
            ...
            if (Const.NoteToVisualKeyDictionary.ContainsKey(num)) Util.Postkey(_hWnd, num, PlayBackground);
        }
    }
}