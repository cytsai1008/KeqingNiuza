...
public static class Util
{
    ...
    {
        if (string.IsNullOrEmpty(key)) throw new ArgumentNullException("Cannot get ascii value of null");
    ...
    {
        if (!User32.UnregisterHotKey(hWnd, 1000)) return false;
        if (!User32.UnregisterHotKey(hWnd, 1001)) return false;
        if (!User32.UnregisterHotKey(hWnd, 1002)) return false;
        return true;
    }
