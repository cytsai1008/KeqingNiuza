...
internal class XunkongApiClient
{
    ...
    private async Task<T> CommonGetAsync<T>(string url) where T : class
    {
        ...
        if (dto is null) throw new XunkongException(-1, "Response body is null.");
        if (dto.Code != 0) throw new XunkongException(dto.Code, dto.Message);
        // warning 不确定是否应该判断响应data为null的情况
        if (dto.Data is null) throw new XunkongException(-1, "Response data is null.");
        return dto.Data;
    }
