...
public class WebDavBackupClient : CloudClient
{
    ...
    public WebDavBackupClient(string username, string password, string url)
    {
        ...
        if (url.Contains("jianguoyun")) IsJianguo = true;
    ...
    public override async Task SyncFiles()
    {
        ...
        {
            result.Dispose();
            foreach (var item in localFiles) await UploadFile(item);
        ...
        var uploadFiles = localFiles.Where(x =>
            remoteFiles.Any(y => x.Name == y.Name && x.LastUpdateTime > y.LastUpdateTime) ||
            !remoteFiles.Any(y => x.Name == y.Name)).Select(x => x);

        var downloadFiles = remoteFiles.Where(x =>
            localFiles.Any(y => x.Name == y.Name && x.LastUpdateTime > y.LastUpdateTime) ||
            !localFiles.Any(y => x.Name == y.Name)).Select(x => x);

        foreach (var item in downloadFiles) await DownloasFile(item);
        foreach (var item in uploadFiles) await UploadFile(item);
    ...
    {
        await _WebDevClient.PutFile($"KeqingNiuza/{info.Name}",
            File.Open($"{UserDataPath}\\{info.Name}", FileMode.Open, FileAccess.ReadWrite));
    }
    ...
    public override async Task BackupFileArchive()
    {
        await Task.Run(async () =>
        {
            ...
            File.Delete(name);
            if (!result.IsSuccessful) throw new Exception(result.Description);
            await PutBackupFileList();
            LastSyncTime = DateTime.Now;
            SaveEncyptedAccount();
        });
    ...
    public override async Task RestoreFileArchive()
    {
        ...
        if (!propfindResponse.IsSuccessful) throw new Exception(propfindResponse.Description);
        if (propfindResponse.Resources.Count < 2) throw new Exception("没有已备份的数据");
        var date = 0;
        ...
        if (!streamResponse.IsSuccessful) throw new Exception(streamResponse.Description);
        var zipArchive = new ZipArchive(streamResponse.Stream);
        var entries = zipArchive.Entries;
        foreach (var entry in entries) entry.ExtractToFile($"{UserDataPath}\\" + entry.FullName, true);
    }
