...
public class WishLogExporter
{
    ...
    ///     获取祈愿记录数据
    /// </summary>
    /// <param name="size">每次Api请求获取几条数据，默认20条，最多20条</param>
    /// <param name="lastId">本地最新的id，获取的祈愿id小于最新id即停止</param>
    /// <returns></returns>
    public async Task<List<WishData>> GetAllLog(int size = 20, long lastId = 0)
    {
        var wishDatas = new List<WishData>();
        await Task.Run(() =>
        {
            var param = new QueryParam {WishType = WishType.Novice, Page = 1, Size = size, EndId = 0};
    ...
    {
        var param = new QueryParam {WishType = type, Page = 1, Size = 20, EndId = 0};
    ...
    public async Task<int> GetUidByUrl()
    {
        ...
        await Task.Run(() =>
        {
            var param = new QueryParam {WishType = WishType.CharacterEvent, Page = 1, Size = 6, EndId = 0};
    ...
    private List<WishData> GetWishLog(QueryParam param)
    {
        ...
        if (result.Retcode != 0) throw new ArgumentException(result.Message);
        if (result.Data.List.Count != 0) list.AddRange(result.Data.List);
        return list;
    }
