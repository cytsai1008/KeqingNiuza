...
public class WishSummary : INotifyPropertyChanged
{
    ...
    public static WishSummary Create(List<WishData> list, bool ignoreFirstStar5Character = false,
        bool ignoreFirstStar5Weapon = false, bool ignoreFirstStar5Permanent = false)
    {
        var summary = new WishSummary
        {
            WishDataList = list
        };
        var sublist = list.Where(x => x.WishType == WishType.CharacterEvent || x.WishType == WishType.CharacterEvent_2)
            .OrderBy(x => x.Id).ToList();
        if (sublist.Any()) summary.CharacterStatistics = GetStatistics(sublist, ignoreFirstStar5Character);
        sublist = list.Where(x => x.WishType == WishType.WeaponEvent).OrderBy(x => x.Id).ToList();
        if (sublist.Any()) summary.WeaponStatistics = GetStatistics(sublist, ignoreFirstStar5Weapon);
        sublist = list.Where(x => x.WishType == WishType.Permanent).OrderBy(x => x.Id).ToList();
        if (sublist.Any()) summary.PermanentStatistics = GetStatistics(sublist, ignoreFirstStar5Permanent);
    ...
    private static void DefineColor(List<StarDetail> list, bool ignoreFirstStar5)
    {
        ...
        if (ignoreFirstStar5) list[0].Brush = "LightGray";
        var brushIndex = 0;
    ...
    private static List<StarDetail> GetDetailList(List<WishData> datas, int star)
    {
        ...
        var list = datas.Where(x => x.Rank == star)
            .Select(x => new StarDetail(x.Name, datas.IndexOf(x), x.Time.ToString("yyyy/MM/dd HH:mm:ss"))).ToList();
        if (list.Any())
        {
