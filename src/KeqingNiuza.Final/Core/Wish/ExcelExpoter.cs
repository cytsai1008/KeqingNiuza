...
public class ExcelExpoter
{
    ...
    {
        var list301 = list.Where(x => x.WishType == WishType.CharacterEvent || x.WishType == WishType.CharacterEvent_2)
    ...
    private void AddWishDataToSheet(List<WishData> datas)
    {
        ...
        for (var i = 0; i < datas.Count; i++)
        {
...
#else
            var chinaTime = TimeZoneInfo.ConvertTime(datas[i].Time,
            ...
            if (datas[i].Rank == 4) cells[i + 2, 1, i + 2, 8].Style.Font.Color.SetColor(Star4Color);
            if (datas[i].Rank == 5) cells[i + 2, 1, i + 2, 8].Style.Font.Color.SetColor(Star5Color);
        }
    }
    ...
    private void AddAllDataToOriginalDataSheet(List<WishData> datas)
    {
        ...
        for (var i = 0; i < datas.Count; i++)
        {
            ...
            if (datas[i].Rank == 4) cells[i + 2, 1, i + 2, 11].Style.Font.Color.SetColor(Star4Color);
            if (datas[i].Rank == 5) cells[i + 2, 1, i + 2, 11].Style.Font.Color.SetColor(Star5Color);
        }
    }
}