...
public class Prediction
{
    ...
    {
        if (n < 1 || n > 90) throw new ArgumentOutOfRangeException();
        if (n < 74)
            return 0.006;
        if (n < 90)
    ...
    {
        if (n < 1 || n > 80) throw new ArgumentOutOfRangeException();
        if (n < 63)
            return 0.007;
        if (n < 74)
    ...
    {
        if (star5Count < 0) throw new ArgumentOutOfRangeException();
        ...
        for (var n = 2; n <= star5Count; n++)
        {
            Array.Clear(den, 0, den.Length);
            Parallel.For(n, 90 * n + 1, i =>
            {
                for (var j = 1; j <= 90 && i - j > 0; j++) den[i - 1] += tmp[i - j - 1] * InitCharacterDensity[j - 1];
            });
            den.CopyTo(tmp, 0);
        }

        dis[0] = den[0];
        for (var i = 1; i < den.Length; i++) dis[i] = dis[i - 1] + den[i];
        return (den, dis);
    }
    ...
    public static (double[] density, double[] distribution) GetWeaponDensityAndDistribution(int star5Count)
    {
        ...
        for (var n = 2; n <= star5Count; n++)
        {
            ...
            {
                for (var j = 1; j <= 80 && i - j > 0; j++) den[i - 1] += tmp[i - j - 1] * InitWeaponDensity[j - 1];
            });
            den.CopyTo(tmp, 0);
        }
    ...
    private static double[] GetInitCharacterDensity()
    {
        ...
        for (var i = 1; i < result.Length; i++)
            result[i] = result[i - 1] / GetCharacterProbability(i) * (1 - GetCharacterProbability(i)) *
                        GetCharacterProbability(i + 1);
        return result;
    }
    ...
    private static double[] GetInitWeaponDensity()
    {
        ...
        result[0] = 0.007;
        for (var i = 1; i < result.Length; i++)
            result[i] = result[i - 1] / GetWeaponProbability(i) * (1 - GetWeaponProbability(i)) *
                        GetWeaponProbability(i + 1);
        return result;
    }
    ...
    private static double[] GetInitSpecifiedWeaponDensity()
    {
        ...
        for (var i = 0; i < InitWeaponDensity.Length; i++) res[i] += 0.375 * InitWeaponDensity[i];
        ...
        for (var i = 0; i < dis.Length; i++) res[i] += dis[i];
...
#if false
        #region These methods are for blazor with single thread
