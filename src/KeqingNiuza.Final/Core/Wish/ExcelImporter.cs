    ...
    private void SortImportedData()
    {
        ...
        for (var i = 2;; i++)
            try
            {
                ...
                if (id != null) data.Id = long.Parse(id);
        ...
        do
        {
            ...
            if (characterQueue.Any()) a = characterQueue.Peek();
            if (weaponQueue.Any()) b = weaponQueue.Peek();
            if (permanentQueue.Any()) c = permanentQueue.Peek();
            if (noviceQueue.Any()) d = noviceQueue.Peek();
            result = a;
            if ((b?.Time ?? DateTime.Now) < (result?.Time ?? DateTime.Now)) result = b;
            if ((c?.Time ?? DateTime.Now) < (result?.Time ?? DateTime.Now)) result = c;
            if ((d?.Time ?? DateTime.Now) < (result?.Time ?? DateTime.Now)) result = d;
