...
public class LocalWishLogLoader
{
    ...
    public static List<WishData> Load(string file)
    {
        ...
        foreach (var group in groups) resultList.AddRange(ComputeWishInfo(WishEventList, group.ToList()));
    ...
    private static List<WishData> ComputeWishInfo(List<WishEvent> WishEventList, List<WishData> subList)
    {
        ...
        for (var i = 0; i < subList.Count(); i++)
        {
            ...
            if (data.WishType == WishType.Permanent || data.WishType == WishType.Novice) data.GuaranteeType = "保底内";
            if (data.Rank == 5)
            {
                tmp = i;
