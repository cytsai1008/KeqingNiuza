...
///     成就计算方法
/// </summary>
internal static class AchievementComputeMethod
{
    private static readonly List<WishEvent> WishEventList;
    ...
    {
        var list = datas.Where(x => x.WishType == WishType.CharacterEvent || x.WishType == WishType.WeaponEvent)
            .GroupBy(x => x.Time.Date).OrderBy(g => g.Key).ToList();
    ...
    public static void 三个胡桃(List<WishData> datas, List<AchievementInfo> infos)
    {
        ...
        {
            var list = datas.Where(x =>
                    x.Time >= item.StartTime && x.Time <= item.EndTime && x.WishType == WishType.CharacterEvent)
                .Select(x => x);
    ...
    {
        var events = WishEventList.Where(x => x.Rank5UpItems.Contains("护摩之杖"));
        foreach (var item in events)
        {
            var list = datas
                .Where(x => x.Time >= item.StartTime && x.Time <= item.EndTime && x.WishType == WishType.WeaponEvent)
                .Where(x => x.Name == "狼的末路" || x.Name == "护摩之杖");
            var langmo = 0;
    ...
    {
        var events = WishEventList.Where(x => x.Rank5UpItems.Contains("松籁响起之时"));
        foreach (var item in events)
        {
            var list = datas
                .Where(x => x.Time >= item.StartTime && x.Time <= item.EndTime && x.WishType == WishType.WeaponEvent)
                .Where(x => x.Name == "风鹰剑" || x.Name == "松籁响起之时");
            var fengying = 0;
            var time = DateTime.Now;
            foreach (var data in list)
                if (data.Name == "风鹰剑")
    ...
    public static void 最后一分钟的奇迹(List<WishData> datas, List<AchievementInfo> infos)
    {
        ...
        DateTime time1 = new(), time2 = new(), time3 = new();
        foreach (var @event in WishEventList)
        {
            var list = datas.Where(x =>
                x.WishType == @event.WishType && x.Time >= @event.EndTime - new TimeSpan(0, 1, 0) &&
                x.Time <= @event.EndTime);
            if (list.Any())
            {
                ...
                if (star5.Any())
                {
                    ...
                    foreach (var item in star5)
                        if (@event.Rank5UpItems.Contains(item.Name))
                        {
                            isup = true;
                            count1++;
                            if (time1 == new DateTime()) time1 = item.Time;
                            break;
                        }
                    ...
                    {
                        count2++;
                        if (time2 == new DateTime()) time2 = star5.First().Time;
                    }
                }
                else
                {
                    count3++;
                    if (time3 == new DateTime()) time3 = list.First().Time;
                }
            }
        }
