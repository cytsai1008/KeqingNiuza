...
public partial class MusicGameView : UserControl, IDisposable, INotifyPropertyChanged
{
    ...
    private readonly IntPtr _hwnd_genshin;

    private readonly IntPtr _hwnd_keqing;
    ...
    private readonly List<RecordingKey> _keys;
    ...
    public MusicGameView()
    {
        ...
        if (RegisterHotKey(_hwnd_keqing)) IsRegisterHotkey = true;
        ...
        _keys = JsonSerializer.Deserialize<List<RecordingKey>>(json);
    ...
    private IntPtr MusicGameHwndHook(IntPtr hWnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        ...
        if (msg == WM_KOTKEY)
            switch (wParam.ToInt32())
            {
                ...
                //开始录制
                case 1006:
                    _hookId = User32.SetWindowsHookEx(WH_KEYBOARD_LL, _hookProc,
                        User32.GetModuleHandle(Process.GetCurrentProcess().MainModule.ModuleName), 0);
                ...
                case 1007:
                    ...
                    try
                    {
                        File.WriteAllText($"{Const.UserDataPath}\\RecordingKeys-{DateTime.Now:yy-MM-dd-HH-mm-ss}.json",
                            json);
                    }
                    catch (Exception ex)
                    {
    ...
    private async void Play()
    {
        ...
        for (;;)
        {
            ...
            while (keyQueue.Any() && keyQueue.FirstOrDefault()?.Time == time) keys.Add(keyQueue.Dequeue());
            ;
            var delay = (int) (time + _releativeTime - sw.ElapsedMilliseconds);
            await Task.Delay(delay > 0 ? delay : 0);
            keys.ForEach(x => User32.PostMessage(_hwnd_genshin, (Msg) x.Operation, (IntPtr) x.Key, (IntPtr) 0));
            Console.WriteLine(sw.ElapsedMilliseconds);
            foreach (var item in keys) Console.Write(item.Key + "  ");
            Console.WriteLine();
            keys.Clear();
            if (!keyQueue.Any()) break;
        }
    ...
    private IntPtr KeyboardHook(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0)
        {
            ...
            if (wParam == (IntPtr) Msg.WM_KEYDOWN || wParam == (IntPtr) Msg.WM_CHAR || wParam == (IntPtr) Msg.WM_KEYUP)
            {
                ...
                _keys.Add(new RecordingKey
                    {Time = _hookSw.ElapsedMilliseconds, Operation = (int) wParam, Key = vkCode});
            }
        }
