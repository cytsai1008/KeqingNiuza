...
using Mapster;
using CharacterInfo = KeqingNiuza.Core.XunkongApi.CharacterInfo;
using Const = KeqingNiuza.Service.Const;
using WeaponInfo = KeqingNiuza.Core.XunkongApi.WeaponInfo;
...
public partial class AboutView : UserControl, INotifyPropertyChanged
{
    ...
    static AboutView()
    {
        TypeAdapterConfig<CharacterInfo, Core.Wish.CharacterInfo>.NewConfig()
            .Map(d => d.Rank, s => s.Rarity)
            .Map(d => d.ElementType, s => s.Element)
            .Map(d => d.Thumb, s => s.ToThumb)
            .Map(d => d.Portrait, s => s.ToPortrait);
        TypeAdapterConfig<WeaponInfo, Core.Wish.WeaponInfo>.NewConfig()
            .Map(d => d.Rank, s => s.Rarity)
            .Map(d => d.Thumb, s => s.ToThumb)
            .Map(d => d.Portrait, s => s.ToPortrait);
    }
    ...
    {
        InitializeComponent();
        TextBlock_Version.Text = "版本：" + Const.FileVersion;
    }

    public bool OverWriteExistedFile
    {
        get => _OverWriteExistedFile;
        set
        {
    ...
    private async void _Button_Download_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            File.WriteAllText("Resource/List/CharacterInfoList.json",
                JsonSerializer.Serialize(localCharacters, Core.CloudBackup.Const.JsonOptions));
            var weapons = await client.GetWeaponInfosAsync();
            var localWeapons = weapons.Adapt<List<Core.Wish.WeaponInfo>>();
            File.WriteAllText("Resource/List/WeaponInfoList.json",
                JsonSerializer.Serialize(localWeapons, Core.CloudBackup.Const.JsonOptions));
            ...
            File.WriteAllText("Resource/List/WishEventList.json",
                JsonSerializer.Serialize(localEvents, Core.CloudBackup.Const.JsonOptions));
