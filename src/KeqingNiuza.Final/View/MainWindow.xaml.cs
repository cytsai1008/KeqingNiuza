...
public partial class MainWindow : Window
{
    ...
    private void Window_Main_Loaded(object sender, RoutedEventArgs e)
    {
        try
        {
            WindowState = Settings.Default.IsWindowMaximized ? WindowState.Maximized : WindowState.Normal;
        }
        catch (Exception ex)
        {
        ...
        // 如果软件窗口超出屏幕边界，则最大化
        if (Native.IsWindowBeyondBounds(ActualWidth, ActualHeight)) WindowState = WindowState.Maximized;
        InitSideMenuChecked();
    }

    private void InitSideMenuChecked()
    {
        if (ViewModel.ViewContent is WishSummaryView) SideMenu_WishSummaryView.IsChecked = true;
    }
    ...
    private async void Button_Cloud_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            var client = await Dialog.Show(new CloudLoginDialog()).Initialize<CloudLoginDialog>(x => { })
                .GetResultAsync<CloudClient>();
            if (client != null)
            {
    ...
    {
        if (File.Exists($"{UserDataPath}\\Account")) File.Delete($"{UserDataPath}\\Account");
    ...
    {
        if (ListView_UserData.SelectedItem == null) return;
    ...
    private async void Button_DeleteUid_Click(object sender, RoutedEventArgs e)
    {
        ...
        Popup_Uid.IsOpen = false;
        var result = await Dialog.Show(new DeleteUidDialog(userData.Uid)).Initialize<DeleteUidDialog>(x => { })
            .GetResultAsync<bool>();
        if (result)
        {
            ...
            if (ViewModel.SelectedUserData == userData)
            {
                ...
                else
                {
                    ...
                    if (File.Exists($"{UserDataPath}\\Config.json")) File.Delete($"{UserDataPath}\\Config.json");
                }
            }

            ViewModel.SaveConfig();
            if (File.Exists(userData.WishLogFile)) File.Delete(userData.WishLogFile);
